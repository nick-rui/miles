...
2. Calls the custom agent (from `--custom-agent-function-path`) to send one or more
   chat requests.
3. Collects worker-assembled `Sample` objects via `OpenAIEndpointTracer.collect_samples`
   (the session server converts records into samples, truncates and merges inside the
   owning worker; records never leave the server).
...
[`sample_assembly.py`](https://github.com/radixark/miles/blob/main/miles/rollout/session/sample_assembly.py)
