...
"""Training-sample assembly: session records -> `Sample` objects, plus the wire codec.
...
- The wire codec (`encode_samples_reply` / `decode_samples_reply`) lives here,
  not in stdlib-only `ipc.py` (imported by the torch-free router) and not in
  `worker.py` (the driver needs the decode side without the registry/backend
  stack). The worker assembles on blank `Sample()` templates and only the
  COMPUTED_FIELDS cross the wire; the driver overlays them onto deepcopies of
  its local input sample. Overlay equivalence with the legacy driver-side
  pipeline requires the input sample to carry dataclass defaults on the fields
  that pipeline evolved in place — see `_assert_overlay_template_defaults`.
"""
...
def compute_samples_from_openai_records(
    ...
    ``tests/fast/rollout/session/test_sample_assembly.py``
...
# ── wire codec: worker-assembled Samples -> reply bytes -> driver overlay ──
